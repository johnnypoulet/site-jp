...
describe('PasteCommand', () => {
  ...
  beforeEach(() => {
    ...
    clipboardService.resetPasteOffest();
  ...
  it('should add the primitives in the clipboard to the list of primitives' , () => {
  ...
  it('should reset offset when the primitives duplicated would be outside the canvas', () => {
    ...
    command = new PasteCommand(clipboardService);
    command.apply(primitives);
    expect(clipboardService.getPasteOffset()).toEqual(ORIGIN);
  });
